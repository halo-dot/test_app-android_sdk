...
import za.co.synthesis.halo.sdk.model.HaloVerificationTokenResult
...
class HaloCallbacks(
        ...
        activity.runOnUiThread {
            ...
            if (result.warnings.isNotEmpty()) {
                for (warning in result.warnings) {
                    Log.e("Warnings", "Warnings: ${warning.errorCode}${warning.details.fold("") { acc, s -> "$acc $s," }}")
                }
            }
        }
    }
    ...
    override fun onHaloTransactionResult(result: HaloTransactionResult) {
        ...
        Log.d(TAG, "resultString: " + HtmlCompat.fromHtml(resultString, HtmlCompat.FROM_HTML_MODE_COMPACT))

        if (result.errorCode.toString().equals("ok", ignoreCase = true)) {
    ...
    private fun String?.format(pattern1: String, pattern2: String): String? {
        ...
        return try {
            SimpleDateFormat(pattern1, Locale.getDefault()).parse(this)?.let { date ->
